...
function selectSQL_SAFE(PDO,SQL,arr,func,func_err)
{  
  PDO.transaction(function(tx){
    tx.executeSql(SQL,arr, func,function(err){     
      func_err(err);      
    }); //tx
  }); //tx
}
...
{
  var arr = new Array();
  for(var i=0,max_i=res.rows.length;i<max_i;i++)
  {  
    arr.push( $.extend(true, {}, res.rows.item(i)) );
  }
  return arr;
}
...
function getDeviceKind(){
  ...
  else if(is_string_like(UA,'%IPAD%'))
	{
		return "IOS";
	}  
}
